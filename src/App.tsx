...
export default function App() {
  ...
  const fetchProfilesList = async () => {
    try {
      let data: Record<string, TagProfile> | null = null;

      // Attempt to fetch directly from Firestore if configured
      if (isFirebaseConfigured) {
        try {
          const querySnapshot = await getDocs(collection(db, "profiles"));
          const profiles: Record<string, TagProfile> = {};
          querySnapshot.forEach((docSnap) => {
            profiles[docSnap.id] = { id: docSnap.id, ...docSnap.data() } as TagProfile;
          });
          if (Object.keys(profiles).length > 0) {
            data = profiles;
          }
        } catch (fErr) {
          console.error("Direct Firestore fetch all profiles failed:", fErr);
        }
      }

      // Fallback to Express API
      if (!data) {
        data = await fetchJsonSafely("/api/profiles");
      }
  ...
  const saveProfileData = async (dataToSave: TagProfile) => {
    ...
    try {
      ...
      // Attempt to save directly to Firestore if configured
      if (isFirebaseConfigured) {
        try {
          const docRef = doc(db, "profiles", finalProfile.id);
          await setDoc(docRef, finalProfile);
        } catch (fErr) {
          console.error("Direct Firestore save failed:", fErr);
        }
      }
      ...
      try {
        ...
        console.warn("Backend API save failed, but direct Firestore/localStorage save succeeded:", apiErr);
      }
  ...
  const handleCreateNewProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isFirebaseConfigured) {
        try {
          const docRef = doc(db, "profiles", sanitizedId);
          await setDoc(docRef, newProfile);
        } catch (fErr) {
          console.error("Direct Firestore create failed:", fErr);
        }
      }
      ...
      try {
        ...
        console.warn("Backend API create failed, but direct Firestore/localStorage save succeeded:", apiErr);
      }
    } catch (err) {
  ...
  const handleDeleteProfile = async () => {
    ...
    try {
      ...
      // Attempt to delete directly from Firestore if configured
      if (isFirebaseConfigured) {
        try {
          const docRef = doc(db, "profiles", profile.id);
          await deleteDoc(docRef);
        } catch (fErr) {
          console.error("Direct Firestore delete failed:", fErr);
        }
      }
      ...
      try {
        ...
        console.warn("Backend API delete failed, but direct Firestore/localStorage delete succeeded:", apiErr);
      }
    } catch (err) {
